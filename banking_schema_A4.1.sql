...
-- 1. Advanced Join Query (3+ tables): Customer Card Details with Type
SELECT DISTINCT 
...
-- 2. Complex Join with Branch and Employee Data
SELECT 
    B.BranchName,
...
-- 3. Subquery: High-Value Customers
SELECT 
    C.Name,
...
-- 4. Correlated Subquery with Window Function
SELECT 
    C.Name,
    L.LoanType,
    L.Amount,
    RANK() OVER (PARTITION BY L.LoanType ORDER BY L.Amount DESC) AS LoanRankInCategory
FROM Customer C
JOIN Loan L ON C.CustomerID = L.CustomerID
WHERE EXISTS (
    SELECT 1 
    ...
    AND A.Balance > L.Amount * 0.1
);

-- 5. Transaction Analysis with Multiple Conditions
SELECT 
    A.AccountID,
    C.Name AS CustomerName,
...
-- Test all views with meaningful columns
SELECT * FROM CustomerLoanSummary;
SELECT * FROM ActiveAccounts WHERE Balance > 5000;
SELECT * FROM TransactionSummary ORDER BY TotalTransactions DESC;